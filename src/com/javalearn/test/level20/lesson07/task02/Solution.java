package com.javalearn.test.level20.lesson07.task02;
...
public class Solution {
    ...
    {
        OutputToConsole outputToConsole = new OutputToConsole(4);
        File your_file_name = File.createTempFile("your_file_name", null);
        outputToConsole.outputToConsole(5);
        ObjectOutputStream outputStream = new ObjectOutputStream(new FileOutputStream(your_file_name));


        outputStream.writeObject(outputToConsole);
        outputStream.close();


        ObjectInputStream inputStream = new ObjectInputStream(new FileInputStream(your_file_name));
        OutputToConsole outputToConsoleLoaded =(OutputToConsole) inputStream.readObject();
        outputToConsoleLoaded.outputToConsole(5);
        inputStream.close();
